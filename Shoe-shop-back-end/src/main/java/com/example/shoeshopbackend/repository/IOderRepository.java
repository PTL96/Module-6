...
public interface IOderRepository extends JpaRepository<Oder, Long> {
    ...
    @Query(value = "SELECT name_product as nameProduct,price_product as priceProduct, avatar_product as avatar, SUM(quantity) as totalQuantity" +
            "            FROM oder " +
            "            GROUP BY product_id " +
            "            ORDER BY SUM(quantity) DESC LIMIT 5", nativeQuery = true)
    List<HotProductDto> hotProduct();
    ...
    @Query(value = "SELECT price_product as priceProduct, name_product as nameProduct, account_id as accounId, oder_id as oderId, date_payment as datePayment,quantity, price_product * quantity as totalPrice\n" +
            ...
            "GROUP BY product_id order by datePayment desc;", nativeQuery = true)
